### Only Both+ proteomics analysis — FIXED VERSION
...
Both_samples <- sample_metadata_FULL$Name[grep("Both", sample_metadata_FULL$Astrocyte)]
...
# Fix "__B" suffix introduced by make_se_parse BEFORE any sorting or alignment
colnames(ProtDataImpFinal) <- gsub("__B$", "_B", colnames(ProtDataImpFinal))
...
# Both transpose objects now derive from the same sorted matrix
...
# SUFFIX FIX
# make_se_parse converts "_B" suffix to "__B"; gsub restores original sample names.
...
ggplot(res_tbl) +
  ...
  ggtitle("Volcano plot MS vs control Both Astrocytes") +
  scale_color_manual(
...
ggsave(paste0(pathResults, "MS_vs_Control_Both_volcano_plot.pdf"), width = 17, height = 17)
ggsave(paste0(pathResults, "MS_vs_Control_Both_volcano_plot.png"), height = 14, width = 15)
