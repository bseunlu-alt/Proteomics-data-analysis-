### Only Both+ proteomics analysis — with integrated validation checks
set.seed(42)
...
data <- ProtData[, which(colnames(ProtData) %in% Both_samples)]
...
sample_metadata <- sample_metadata[grep("Both", sample_metadata$Astrocyte), ]
...
if (length(unique_replicates) == 1 && unique_replicates == "1") {
  ...
  # Auto-patch if sample_metadata already has a Replicate column
...
clinical_metadata <- clinical_metadata[clinical_metadata$ID %in% Both_samples, ]
...
# PCA PLOT
# make_se_parse converts "_B" suffix → "__B"; gsub restores original names.

# Apply suffix fix
rownames(adjusted_data) <- gsub("__B$", "_B", rownames(adjusted_data))
...
# Checks both adjusted_data and the prodat_adj column names that are fixed later.
...
# After 2^ the values should be large intensities; after limmaDE's log2 transform
# they return to log-scale — the round-trip is mathematically neutral but
# confirming the starting scale prevents hard-to-debug mistakes.
...
# Restore original sample names in column headers ("__B" → "_B")
colnames(prodat_adj$tab) <- gsub("__B$", "_B", colnames(prodat_adj$tab))

# CHECK 2b: Suffix fix on prodat_adj column names
# make_se_parse introduced "__B" in column headers; confirm it is fully resolved
# before the dimension/order stopifnot checks below.
...
# Same rationale as CHECK 3a but applied to the covariate-adjusted matrix.
...
write.csv(all_DEGs,
          paste0(pathResults, "all_sig_DEGs_Both_revised.csv"),
